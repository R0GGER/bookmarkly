...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        $updatedBookmark = [
            ...
            'favorite' => ($_POST['categoryId'] === 'favorites')
        ];

        $db->updateBookmark($updatedBookmark);
        header('Location: admin.php');
        exit();
    } catch (Exception $e) {
        $error = $e->getMessage();
...
<head>
    ...
    <title>Bookmarkly - <?php echo $lang['edit_bookmark']; ?></title>
...
<body>
    <div class="form-container">
        <div class="admin-header">
            <h2><?php echo $lang['edit_bookmark']; ?></h2>
            ...
            <div class="form-group">
                ...
                <div style="display: flex; gap: 0.5rem;">
                    <button type="button" class="icon-select-btn" onclick="openIconModal()">
                        <img id="selectedIcon" src="<?php echo htmlspecialchars($bookmark['icon']); ?>" 
                             class="icon-preview" 
                             style="<?php echo empty($bookmark['icon']) ? 'display: none;' : ''; ?>">
                        <span><?php echo $lang['choose_icon']; ?></span>
                    </button>
                    <input type="text" id="icon" name="icon" required style="flex: 1;" 
                           value="<?php echo htmlspecialchars($bookmark['icon']); ?>">
                </div>
            </div>
            
            <div class="form-group">
                <label for="categoryId"><?php echo $lang['category']; ?></label>
                <select id="categoryId" name="categoryId" required>
                    <option value="favorites" <?php echo $bookmark['favorite'] ? 'selected' : ''; ?>>
                        Favorites
                    </option>
                    <?php foreach ($categories as $category): ?>
                        <option value="<?php echo htmlspecialchars($category['id']); ?>"
                                <?php echo (!$bookmark['favorite'] && $bookmark['categoryId'] === $category['id']) ? 'selected' : ''; ?>>
    ...
    <!-- Voeg deze HTML toe voor het form element -->
    ...
    <!-- Voeg het JavaScript toe voor het einde van de body tag -->
    <script>
    let homarrIcons = [];
    ...
    // Voeg ook de upload functionaliteit toe
